...
class AQIVisualizer:
    ...
    def create_folium_map(self, wf_gdf: gpd.GeoDataFrame, center=[39.0, -105.5], zoom_start=6):
        """
        Creates an interactive Folium map centered on Colorado,
        adds markers for each wildfire event in wf_gdf.
        ...
        if wf_gdf.empty:
            self.logger.warning("The provided GeoDataFrame is empty. No markers will be added.")
        else:
            # 2) For each event in the GeoDataFrame, add a marker with a popup
            for idx, row in wf_gdf.iterrows():
                ...
                folium.Marker(
                    location=[lat, lon],
                    popup=popup_html,
                    tooltip=f"Wildfire: {row.get('acq_date', 'N/A')}"
                ).add_to(m)

        # 3) Save Folium map to HTML
        map_path = os.path.join(self.output_dir, f"folium_wildfire_aqi_map_{self.start_year}_{self.end_year}.html")
        m.save(map_path)
        self.logger.info(f"Folium wildfire-AQI map saved to {map_path}.")